...
[MCP](https://modelcontextprotocol.io) server exposing the Gcal SDK as
two agent tools — `gcal_list` and `gcal_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/gcal-mcp)
make build
...
claude mcp add --scope user gcal \
  -- /absolute/path/to/gcal-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./gcal-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// gcal_list: first page of records
{ "entity": "event" }
{ "entity": "event", "query": { } }

// gcal_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/gcal-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user gcal \
     -- "$PWD"/dist/*/gcal-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `gcal_list` and `gcal_load` tools now appear
   in new sessions. Ask the agent to *"list event using gcal"*
   and it calls `gcal_list` with `{"entity":"event"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `gcal_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `gcal_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `GCAL_APIKEY` | API key sent with every request. |
| `GCAL_BASE` | Optional override of the API base URL. |
...
```sh
./gcal-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"gcal_load","arguments":{"entity":"event","query":{"id":1}}}}'
```
