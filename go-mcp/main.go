// Command Gcal-mcp is an MCP server exposing the Gcal
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Gcal_list  — list records of an entity
//	Gcal_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: GCAL_APIKEY carries the API key and
	// GCAL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("GCAL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("GCAL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewGcalSDK(opts)
