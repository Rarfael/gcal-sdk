...
class GcalTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            GcalTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = GcalTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = GcalTestRunner.getenv("GCAL_TEST_LIVE")
        override = GcalTestRunner.getenv("GCAL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = GcalTestRunner.getenv(key)
        ...
        explain = GcalTestRunner.getenv("GCAL_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["GCAL_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if GcalTestRunner._test_control is not None:
            return GcalTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                GcalTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            GcalTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = GcalTestRunner.load_test_control()
...
def load_env_local():
    GcalTestRunner.load_env_local()


def env_override(m):
    return GcalTestRunner.env_override(m)


def entity_data(v):
    return GcalTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return GcalTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return GcalTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return GcalTestRunner.load_test_control()


def live_delay_ms():
    return GcalTestRunner.live_delay_ms()
