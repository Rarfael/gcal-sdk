...
from gcal_sdk.gcal_types import (
    Event,
    EventLoadMatch,
    EventListMatch,
    EventCreateData,
...
class EventEntity:
    ...
    def update(self, reqdata: EventUpdateData, ctrl=None) -> Event:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> Event:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.Event().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
