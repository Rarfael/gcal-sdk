...
class FtCtrl extends GcalControl
{
...
class FtClient
{
    ...
    private GcalUtility $utility;

    public function __construct(GcalUtility $utility, array $options)
    {
    ...
    public function get_utility(): GcalUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (GcalContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new GcalUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (GcalContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = GcalConfig::make_feature($name);
    ...
    private static function buildUrl(GcalSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof GcalError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new GcalResponse($fetched);
        }
    ...
    private function fail(GcalContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (GcalContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
