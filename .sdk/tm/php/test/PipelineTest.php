...
{
    private static function utility(): GcalUtility
    {
        return new GcalUtility();
    }
    ...
    private static function ctx(array $over = []): GcalContext
    {
        ...
        $ctx->op = new GcalOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof GcalError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new GcalOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = GcalMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new GcalOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = GcalMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new GcalOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = GcalMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GcalSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = GcalMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new GcalResponse([]),
            'result' => new GcalResult([]),
        ]);
        [, $err] = GcalMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new GcalSpec(['step' => 's']),
            'response' => new GcalResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new GcalResult([]),
        ]);
        [, $err] = GcalMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new GcalSpec(['step' => 's']),
            'response' => new GcalResponse(self::resp(200, ['v' => 1])),
            'result' => new GcalResult([]),
        ]);
        [, $err] = GcalMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new GcalSpec(['step' => 's']),
            'response' => new GcalResponse(self::resp(200, ['v' => 2])),
            'result' => new GcalResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        GcalMakeResponse::call($ctx);
    ...
    {
        $preset = new GcalResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new GcalSpec([]),
            'response' => new GcalResponse([]),
            'result' => new GcalResult([]),
        ...
        [$got, $err] = GcalMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new GcalResult([])]);
        [, $err] = GcalMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new GcalSpec([]), 'result' => null]);
        [, $err] = GcalMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GcalSpec(['step' => 's']),
            'result' => new GcalResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new GcalOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GcalMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new GcalSpec(['step' => 's']),
            'result' => new GcalResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new GcalOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = GcalMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new GcalResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new GcalSpec([]),
            'result' => new GcalResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = GcalMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new GcalError('boom', 'boom');
        $utility->fetcher = function (GcalContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GcalSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = GcalMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (GcalContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = GcalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(GcalResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (GcalContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new GcalSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = GcalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new GcalResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new GcalSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = GcalMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = GcalMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new GcalResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, GcalDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new GcalResult(['ok' => false])]);
        $this->expectException(GcalError::class);
        GcalDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new GcalResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', GcalMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new GcalBaseFeature();
        $b = new GcalBaseFeature();
        GcalFeatureAdd::call($ctx, $a);
        GcalFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): GcalBaseFeature
    {
        $f = new GcalBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): GcalContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new GcalSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new GcalResponse(['headers' => null]),
            'result' => new GcalResult([]),
        ]);
        GcalResultHeaders::call($ctx);
