...
class GcalMakeError
{
    public static function call(?GcalContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new GcalContext([], null);
        }
        $op = $ctx->op ?? new GcalOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new GcalResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof GcalError) ? $err->msg : (string)$err;
