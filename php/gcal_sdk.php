...
class GcalSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = GcalHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = GcalHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!GcalFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, GcalFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return GcalUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = GcalHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = GcalHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = GcalHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = GcalHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new GcalError("graphql_error",
                "GcalSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
