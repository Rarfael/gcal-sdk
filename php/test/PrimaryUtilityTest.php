...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ...
        $ctx = new GcalContext($ctxmap, null);
    ...
    private static function fixctx(GcalContext $ctx, GcalSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?GcalError
    {
        if ($m === null) {
            return null;
        }
    ...
    {
        $hook_client = GcalSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends GcalBaseFeature {
            public $hook_fn = null;
            public function TestHook(GcalContext $ctx): void
            {
    ...
    {
        $init_client = GcalSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends GcalBaseFeature {
            public $init_fn = null;
            ...
            public function init(GcalContext $ctx, array $options): void
            {
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new GcalSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = GcalSDK::test(null, $setup_opts);
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = GcalSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof GcalError) ? $result->err->msg : (string)$result->err;
