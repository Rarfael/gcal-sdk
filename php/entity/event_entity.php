...
class EventEntity
{
    ...
    {
        if ($args) {
            $this->_data = GcalHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = GcalHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws GcalError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = GcalHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws GcalError on failure (item-5 convention).
     */
     ...
     *
     * @param EventUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed EventUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Event|array The updated Event as an assoc-array at the
     *   SDK boundary; throws GcalError on failure (item-5 convention).
     */
    ...
    /**
     * Remove an Event matching the given criteria.
     *
     * @param EventRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; EventRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Event|array The removed Event as an assoc-array at the
     *   SDK boundary; throws GcalError on failure (item-5 convention).
     */
