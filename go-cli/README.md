...
boru-driven command-line client **and** interactive REPL for the Gcal
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/gcal-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/gcal-cli
   ```
...
Run with no arguments to open a REPL (prompt `gcal>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/gcal-cli`. |
...
(`./gcal-cli <expr>`) and interactively (the REPL), and expressions compose the
