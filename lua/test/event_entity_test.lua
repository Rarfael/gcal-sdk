...
describe("EventEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set GCAL_TEST_EVENT_ENTID JSON to run live")
      return
    end
    ...
    local event_ref01_resdata_up0_result, err = event_ref01_ent:update(event_ref01_data_up0_up, nil)
    assert.is_nil(err)
    local event_ref01_resdata_up0 = helpers.to_map(type(event_ref01_resdata_up0_result) == 'table' and event_ref01_resdata_up0_result.data_get and event_ref01_resdata_up0_result:data_get() or event_ref01_resdata_up0_result)
    assert.is_not_nil(event_ref01_resdata_up0)
    assert.are.equal(event_ref01_resdata_up0["id"], event_ref01_data_up0_up["id"])
    assert.are.equal(event_ref01_resdata_up0[event_ref01_markdef_up0_name], event_ref01_markdef_up0_value)
    ...
    -- REMOVE
    local event_ref01_match_rm0 = {
      id = event_ref01_data["id"],
    }
    local _, err = event_ref01_ent:remove(event_ref01_match_rm0, nil)
    assert.is_nil(err)
    ...
    assert.is_nil(err)
    assert.is_table(event_ref01_list_rt0_result)

    local not_found_item = vs.select(
      runner.entity_list_to_data(event_ref01_list_rt0_result),
      { id = event_ref01_data["id"] })
...
function event_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("GCAL_TEST_EVENT_ENTID")
