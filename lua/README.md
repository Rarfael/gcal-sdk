...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Event()` — each with the same small set of operations (`list`, `load`, `create`, `update`, `remove`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
-- Create
local created, err = client:Event():create({ created = "example_created", description = "example_description" })
if err then error(err) end
